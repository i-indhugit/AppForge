...
const CustomPipelineNode = ({ data }: any) => {
...
export default function PipelineFlow({
  ...
  const nodes: Node[] = useMemo(() => {
    const stages = [
      ...
      { key: "schema", label: "Stage 4: Schema Generator" },
      { key: "validation", label: "Stage 5: Validation Engine" },
      { key: "repair", label: "Stage 6: Repair Engine" },
      { key: "verification", label: "Stage 7: Execution Verification" },
      { key: "runtime", label: "Stage 8: Runtime Generator" },
    ];
  ...
  const edges: Edge[] = useMemo(() => {
    const edgeIds = ["intent", "ast", "ir", "architecture", "schema", "validation", "repair", "verification", "runtime"];
    const result: Edge[] = [];
    ...
    for (let i = 0; i < edgeIds.length - 1; i++) {
      ...
      const sourceStatus = stageStatuses[source];
